...
df_print: paged
params:
  name: "MS023 48h Chromatin_Nucleoplasm_Cytoplasm"
  protein_data_file: "C:\\Users\\david\\OneDrive\\Manuscripts\\DeAngelo\\2024 PRMT5 SNRPB Productive mRNA Release\\In Progress\\Analysis\\Proteome_FractionatedA549_PRMTi_MaximMaron2023\\LabelFree_Master_raw_2023_MaximMaron_PRMTiFractionation.txt"
  experimental_design_file: "C:\\Users\\david\\OneDrive\\Manuscripts\\DeAngelo\\2024 PRMT5 SNRPB Productive mRNA Release\\In Progress\\Analysis\\Proteome_FractionatedA549_PRMTi_MaximMaron2023\\experimental_design\\experimental_design - MS023 48h Chromatin Nucleoplasm Cytoplasm.csv"
  proteins_to_test_file: "C:\\Users\\david\\OneDrive\\Manuscripts\\DeAngelo\\2024 PRMT5 SNRPB Productive mRNA Release\\In Progress\\Analysis\\Proteome_FractionatedA549_PRMTi_MaximMaron2023\\proteins_to_test.txt"
  threshold_for_filtering: 0 #Set to 0 for very stringent
  fraction_filtered: 0.75 #Set at 0 to allow allow missing values, as close to 1 as possible for no missing values
  ...
  n_clusters: 6
  label_number: 20 
  column_order: "DMSO.CHR"
  imputation_methods: "GSimp"
  ...
  save_outputs: TRUE
  output_dir: "C:\\Users\\david\\OneDrive\\Manuscripts\\DeAngelo\\2024 PRMT5 SNRPB Productive mRNA Release\\In Progress\\Analysis\\Proteome_FractionatedA549_PRMTi_MaximMaron2023\\DE_output-MS023"
 ...
 if (params$save_output) {
  ...
  if (params$save_output) {
       for (contrast in contrasts) {
        # Save results to CSV
         print(contrast)
        write.csv(df_wide, file.path(output_dir,"\\", paste0("DE_", params$name, "_Imputation-", imputation_methods, "_Cutoff-", params$threshold_cutoff, "_StatTest-", params$fdr_correction, "_DEP2-limma.csv")), row.names = FALSE)
       }
  }
...
extract_centered_values <- function(se, protein_name, output_dir) {
  ...
  write.csv(centered_wide, output_filepath, row.names = FALSE)
...
output_dir <- "single_protein_analysis_fractions"
...
for (i in 1:nrow(proteins_to_test)) {
  ...
  tryCatch({
    ps <- plot_single(de_se, proteins = c(protein_name), type = "centered")
...
```{r ORA analysis, eval = params$perform_post_analysis}
  ...
  #############GO
#res_ora_GO <- DEP2::test_ORA(de_se, by_contrast = T, contrasts = c("DMSO.NUC_vs_DMSO.CYTO",  "MS023.48H.NUC_vs_MS023.48H.CYTO", "DMSO.CHR_vs_DMSO.CYTO", "MS023.48H.CHR_vs_MS023.48H.CYTO", "DMSO.CHR_vs_DMSO.NUC", "MS023.48H.CHR_vs_MS023.48H.NUC"), species = "Human", type = "GO")

res_ora_GO <- DEP2::test_ORA(de_se, by_contrast = T, contrasts = c("MS023.48H.CHR_vs_DMSO.CHR"), species = "Human", type = "GO")
...
print(enrich_plotGO_MF)
    ...
    if (params$save_output) {
      ...
      pdf("enrich_plotGO-Chr-MS023-ALL GO.pdf", width = 6, height = 8)
      print(enrich_plotGO_ALL)
      dev.off()
    }

#############KEGG
res_ora_KEGG <- DEP2::test_ORA(de_se, by_contrast = T, contrasts = c("DMSO.NUC_vs_DMSO.CYTO",  "MS023.48H.NUC_vs_MS023.48H.CYTO", "DMSO.CHR_vs_DMSO.CYTO", "MS023.48H.CHR_vs_MS023.48H.CYTO"), species = "Human", type = "KEGG")
