...
process_data <- function(file_path, type) {
  ...
  data <- data %>%
    ...
    dplyr::select(-Geneid)
  # Apply cutoff of 30 counts in any 3 samples (e.g. should be truly present in at least one fractions/conditions   )
  data <- data %>%
    dplyr::filter(rowSums(dplyr::select(., starts_with("DMSO") | starts_with("GSK591")) >= 30) >= 3)
  return(data)
}
